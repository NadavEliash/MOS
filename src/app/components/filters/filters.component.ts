...
export class FiltersComponent implements OnChanges {
    ...
    if (changes['filterGroupsInput']) {
      ...
      if (filterGroups) {
        ...
        if (isMultiMeasureGraph) {
          ...
          // Single measure mode: normal behavior
          filterGroups.forEach((group: InputFilterGroup) => {
            if (group.filter.labels?.some(label => label.data.checked)) {  
              group.filter.expanded = true;
            }
  ...
  toggleMeasure(measure: MeasureView) {
    ...
    if (isMultiMeasureGraph) {
      // In multi-measure mode, clicking any measure collapses all and clears graph
    ...
    // Check if measure is currently open
    if (measure.expanded) {
      // Measure is open: close it
      measure.expanded = false;
      
      const expandedMeasures = this.measures.filter(m => m.expanded);
      if (expandedMeasures.length > 0) {
        // There's more than one expanded, show the first remaining one
        this.selectMeasure.emit(expandedMeasures[0].id);
      } else {
        // No other measures expanded
        this.selectMeasure.emit('');
      }
    } else {
      // Measure is closed: try to open it
      ...
      if (!otherExpanded) {
        ...
        // Another measure is open, check relations
